...
/** Convert RoutePoints to a minimal GPX string for client-side caching */
function pointsToGpx(points: RoutePoint[], name: string): string {
  const trkpts = points.map(p =>
    `    <trkpt lat="${p.lat}" lon="${p.lon}"><ele>${p.elevationM.toFixed(1)}</ele></trkpt>`
  ).join('\n');
  return `<?xml version="1.0" encoding="UTF-8"?>
<gpx version="1.1" creator="CarbMaps" xmlns="http://www.topografix.com/GPX/1/1">
...
router.post('/analyze-route/:id', async (req: Request, res: Response): Promise<void> => {
  ...
  if (isNaN(intensityN) || intensityN < 55 || intensityN > 100) { res.status(400).json({ error: 'intensity must be 55–100' }); return; }
  ...
  try {
    ...
    const gpxData = pointsToGpx(points, route.name ?? `Strava route #${id}`);
    res.json({ rideId, pacing, climbs, nutrition, weather, routePoints: samplePoints(points, 500), gpxData });
  } catch (err: any) {
